...
- `gc <canvasUrl>` — the browser half of the remove coupling: deleting a tile in
  ...
  **1-hour grace** (covers a ⌘Z), anchored at the first gc run that noticed the
  ...
  Grace state: `godmode/.canvas-gc.json` (local only).
...
batch, migrated lazily from KV the first time a pre-rewrite board is touched.
...
at the URL you navigated to. Anti-echo = `isTrusted` filter + a 400ms quiet window
...
→ 7m30s). At zero the pill and digits shake, read a red 00:00 for everyone, and a
synthesized chime rings (same per-user volume/mute as the music). The clock renders
...
engine, not when you're driving a board. Each gotcha was bought with a real bug;
...
- **The main toolbar:** tool state is one `TOOL` object (`setTool()`), sub-toolbars
  ...
  the tool letters, so ⌘C never toggles comments. The eraser deletes whole `draw`
