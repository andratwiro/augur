...
| **Back-office config forms** (Page) | `pages/bo-project-phase/` | `.gv-bo-formhead`/`-subhead`/`-caption`/`-qlabel`/`-help` · `.gv-bo-select` · `.gv-bo-banner` · `.gv-bo-methods`/`.gv-bo-methodcard`(`.is-selected`) · `.gv-bo-togglerow` · `.gv-bo-tags`/`.gv-bo-tag` · `.gv-bo-imageup`/`.gv-bo-dropzone` · `.gv-bo-table`/`-toolbar`/`-search`/`-pager` · `.gv-bo-pane`/`-card`/`-empty`/`-eventrow` · `.gv-btn.success` `.gv-btn.size-s` | **The deep project-configuration editor**, source-grounded on the live captures (`bo-phase-setup`, `bo-project-general/-audience/-events`) and verified via the `bo-project-phase/*` checkpoints. One clickable page with all six project top-tabs wired to swap panels: **General** (full settings form — multiloc fields, tags, area/content radios, image uploads), **Timeline → phase → Setup** (the rich method picker, anonymity, action toggles, likes/dislikes, similar-input detection, available views, sorting, attachments), **Audience** (participants data table + pager), **Messaging** (empty state), **Events** (event list). Pure assembly on `.gv-bo` primitives + the new config classes. |
