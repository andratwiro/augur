...
//            between someone and seeing their own work.
...
async function publish() {
  ...
  const code = await new Promise((resolve) => {
    const p = spawn(process.execPath, [path.join(ROOT, "scripts", "publish.mjs"), "--space", SPACE],
...
async function reconcile() {
  warn("someone else shipped first — reconciling");
