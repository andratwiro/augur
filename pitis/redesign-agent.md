...
Real reference imagery + behaviour live in `figpals/reference/` (internal, never
...
- `figpals/research.md` — written findings (style + behaviour breakdown).
...
- `figpals/figpal.js` — **the engine** (single source of truth). Contains the
  `PALETTE`, the SVG generators (`svgUpright`, `svgSleep`), `svg()`, the
  `mount()` follow/animation state machine + injected CSS keyframes, and
  `auto()`/`reveal()`/`hide()`.
- `figpals/index.html` — the adopt/customize page (uses the engine).
- `build.js` injects `/figpal.js` into every shell page via `FigPal.auto()` and
  copies the engine to the dist root. **Don't change the wiring.**

**HARD CONSTRAINTS (do not break — other code depends on them):**
- Keep the public API intact: `window.FigPal = { PALETTE, svg, loadConfig,
...
2. **Render your sprite** — there's a harness at
   `figpals/reference/_sprite-test.html` (loads `../figpal.js`, lays out states).
   Screenshot it headless and Read the PNG:
   ```
   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless \
     --disable-gpu --hide-scrollbars --force-device-scale-factor=2 \
     --window-size=600,460 --screenshot=figpals/reference/_sprite-shot.png \
     "file:///Users/rob/Documents/go-vocal-prototypes/figpals/reference/_sprite-test.html"
   ```
...
3. **Check motion** — static shots won't reveal jitter/easing. Build a motion
   harness (`_motion-test.html`) that calls `FigPal.mount()` then synthesises a
...
- Commit only your paths (`figpals/figpal.js`, `figpals/index.html`, and
