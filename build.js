...
// ── Shared chrome bundle (P1) ─────────────────────────────────────────────
// The script+style chrome shell() emits is byte-identical on every page. Ship it
// ONCE as a content-hashed, immutable asset instead of re-inlining ~187 KB into
// every document. Per-page bits (appChrome markup, spaceContextScript data) stay
// inline; the bundle loads `defer`, so it runs AFTER those inline data scripts —
// which only ASSIGN window data, never depend on bundle behaviour running first.
const CHROME_CSS_BODY = `${FONT_CSS}${PAGE_CSS}${NAV_CSS}${TABBAR_CSS}`;
...
const CHROME_CSS_NAME = `_chrome.${UI_VERSION}.${h8(CHROME_CSS_BODY)}.css`;
const CHROME_JS_NAME = `_chrome.${UI_VERSION}.${h8(CHROME_JS_BODY)}.js`;
const CHROME_STAMP = h8(CHROME_JS_BODY + CHROME_CSS_BODY);

// ── Service worker source (P0) ────────────────────────────────────────────
// The pure decision logic lives in src/sw-logic.mjs (unit-tested); here we strip
// its `export`s and append the runtime wiring, so the SHIPPED worker and the
// TESTED logic are one source. BUILD_STAMP namespaces the cache so a new build
// evicts old cached shells on activate. Kill-switch: replace the wiring below with
// an unregister()+caches-clear tombstone and pin-bump (see deploy shell SETUP.md).
const SW_LOGIC_SRC = readFileSync(path.join(ROOT, "src", "sw-logic.mjs"), "utf8")
  .replace(/^export\s+/gm, "");
const SW_SRC = `/* Augur service worker — generated by build.js; logic in src/sw-logic.mjs. */
const BUILD_STAMP = ${JSON.stringify(CHROME_STAMP)};
const CACHE = "augur-v" + BUILD_STAMP;
${SW_LOGIC_SRC}
...
self.addEventListener("fetch", (e) => {
  ...
  const strat = swDecision({
    method: req.method,
    sameOrigin: url.origin === self.location.origin,
    mode: req.mode,
    path: url.pathname,
  });
  if (strat === "swr") e.respondWith(staleWhileRevalidate(req));
  else if (strat === "cache-first") e.respondWith(cacheFirst(req));
  // 'passthrough' → no respondWith; the browser handles it normally.
});
async function staleWhileRevalidate(req) {
  const cache = await caches.open(CACHE);
  const cached = await cache.match(req);
  const net = fetch(req).then((res) => {
    if (cacheEligible(res.status, res.headers.get("Cache-Control"))) cache.put(req, res.clone());
    else cache.delete(req); // logged out / gate / error → drop any stale copy
    return res;
  }).catch(() => null);
  if (cached) { net.catch(() => {}); return cached; }
  return (await net) || new Response("Offline", { status: 503, headers: { "Content-Type": "text/plain" } });
}
...
<head>
  ...
  <link rel="preload" href="/fonts/inter-latin-wght-normal.woff2" as="font" type="font/woff2" crossorigin />
  <link rel="preload" href="/${CHROME_JS_NAME}" as="script" />
...
async function main() {
  ...
  await fs.writeFile(path.join(DIST, CHROME_CSS_NAME), CHROME_CSS_BODY, "utf8");
  await fs.writeFile(path.join(DIST, CHROME_JS_NAME), CHROME_JS_BODY, "utf8");
  await fs.writeFile(path.join(DIST, "sw.js"), SW_SRC, "utf8");
