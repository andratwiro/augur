...
test("end to end through the CLI: start → answer → grade → apply, in a copy of the seed", () => {
  const ws = workspace();
  ...
  // grade refuses the skeleton
  assert.throws(() => run("grade"), /Command failed/);

  fs.writeFileSync(path.join(ws, "canon", "canon.json"), JSON.stringify(answer(), null, 2));
  assert.match(run("grade"), /PASS/);
  ...
  assert.match(run("grade", "--space", "."), /PASS/);
