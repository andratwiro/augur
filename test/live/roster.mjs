...
async function exportRoster() {
  ...
  return doc.families["users:roster"] || { add: {}, remove: [] };
}
async function importRoster(roster) {
  const r = await fetch(`${origin}/__publish/_state/import`, {
    method: "POST", headers: { ...headers, "content-type": "application/json" },
    body: JSON.stringify({ format: 1, families: { "users:roster": roster } }),
  });
...
const cur = await exportRoster();
if (op === "show") {
  console.log(JSON.stringify({ add: Object.keys(cur.add || {}), remove: cur.remove || [] }, null, 2));
} else if (op === "add") {
  if (!fs.existsSync(SNAP)) fs.writeFileSync(SNAP, JSON.stringify(cur, null, 2));
  const add = { ...(cur.add || {}) };
  ...
  const res = await importRoster({ add, remove: (cur.remove || []).filter((e) => !Object.keys(add).includes(e)) });
  console.log(`added ${Object.keys(PERSONAS).length} personas; import: ${JSON.stringify(res)}; snapshot ${SNAP}`);
} else if (op === "remove") {
  ...
  const res = await importRoster({ add: { ...before.add, ...add }, remove: before.remove || [] });
  console.log(`removed personas; import: ${JSON.stringify(res)}`);
} else {
