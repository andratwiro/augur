/* ══════════════════════════════════════════════════════════════════════════
   BLUE RAIL EXPLORER — experimental, prototype-local (v3 only, DETACHED FROM CANON).
   Self-contained: injects its own <style> + a top-centre chip and wires it up, so
   index.html needs only a single <script src> include (minimises clashes with the
   other agents editing this file). Canon (assets/govocal-bo.css → .gv-bo-side) is
   untouched; v2 keeps the shipped rail for reference.

   Premise (agreed with Rob): while editing a project you don't NAVIGATE the global
   rail items — admins/PMs know what's there — so every option KEEPS them and fixes
   the two real faults: icon CONTRAST and on-hover LABEL/context. Five takes:
     1 Tooltip          — brighter icons + a label flyout on hover (ship-now baseline)
     2 Hover drawer     — rail expands to a labelled drawer on hover (kills the toggle)
...
(function () {
  ...
  /* 4 · RECEDE & WAKE — the agreed insight, literal: in project context the global
     rail is dormant (dim) EXCEPT the active item; the whole rail wakes to full
     contrast + tooltips on hover/focus. */
  ...
  /* 5 · GROUPED & ACCENT — a designed pass: brighter icons, a left accent bar + light
     wash on the active item (not the heavy black chip), a hairline between the primary
     group and the utility cluster, plus the shared tooltip. */
  ...
  var OPTS = {
    1: ['Tooltip', '— brighter icons + a hover label'],
  ...
  function build() {
    // styles
    if (!document.getElementById('v3-rail-experiments')) {
      var s = document.createElement('style');
      s.id = 'v3-rail-experiments';
      s.textContent = CSS;
      document.head.appendChild(s);
    }
    // default treatment on the rail
    var rail = document.querySelector('.gv-bo-side');
