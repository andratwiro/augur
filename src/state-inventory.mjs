...
export const STATE_INVENTORY = Object.freeze([
  ...
  // ── identity ───────────────────────────────────────────────────────────────
  {
    id: "users:sessionkeys:", store: "kv", kind: "prefix", to: "workspace",
    why: "Per-person session-binding keys, ONE RECORD PER PERSON under a digest of the address (`users:sessionkeys:<sha256 of the lowercased address>` → `{key}`, with `key: null` for a clear). NOT a credential and never checked against anything a person types — it is the value the session cookie HMACs, split out of the password hash so that ending a session and changing a credential can be separate acts. One record each because the shared document it replaced was rebuilt read-modify-write over a cached read, and rotating one person could write another's OLD key back (`B-sessionkeys-lost-update`); a record is written blind and nobody else's write can carry it. Workspace-destined rather than account-destined because a session belongs to one workspace host, and the credential that opens several does not. Losing it in a copy signs that workspace's people out once, which is recoverable; carrying it to the WRONG workspace would let a cookie cross a boundary, which is not.",
  },
  {
    id: "users:sessionkeys", store: "kv", kind: "key", to: "workspace",
    why: "The RETIRED shared session-key document — every person's key in one map — that `users:sessionkeys:` replaced. Still READ, never written: a person with no record of their own is looked up here, which is how a cookie minted before the per-person records existed keeps verifying after the deploy that introduced them, with nothing backfilled. A person leaves it the first time something rotates or clears their key, because a record (a cleared one included) shadows their entry. It is carried in a copy for the same reason it is read: without it, everybody not yet rotated is signed out once on the destination.",
  },
  {
    id: "users:firstrun", store: "kv", kind: "key", to: "workspace",
    why: "Which addresses have already been shown the first-run surface (the FIRST_RUN instance flag), one map of address → first-landing stamp. Workspace-destined because the property it records is once-only PER WORKSPACE MEMBERSHIP, and it has to be durable rather than cookie-held: a second device for the same person asks this same map. Losing it in a copy re-shows one placeholder page to each person once, which is the safe way for it to be wrong; carrying it to the WRONG workspace would silently swallow that workspace's welcome.",
  },
  {
  ...
  // ── R2, already where it belongs ───────────────────────────────────────────
  {
    ...
    why: "The WORKSPACE SEGMENT (`B-bundle-store-tenancy`, then `B-identity-kv-write-segmentation`). On a deployment that resolves the workspace from the Host, a segmented family is written under `t/<workspace>/` — so `t/` is not a family of its own, it is where those families live, and it carries nothing the entries elsewhere do not describe. ⚠️ IT NOW SPANS BOTH STORES. In R2 it is the three bundle families (`bundleKey`); in KV it is the identity documents — `users:roster`, `users:roles`, `users:names`, `users:avatars`, `users:spaces`, `users:invites`, `users:lastseen:`, `users:sessionkeys:`, `users:sessionkeys`, `users:firstrun`, `publish:tokens`, `spaces:icons`, `spaceicon:`, `avatar:` and `mail:suppressed` (`identityKey`). The entry is filed under R2 because that is where the segment was invented and it is one concept, not two: a live key beginning `t/` in either store is a segmented copy of a family named elsewhere in this list. A deployment serving one workspace writes no key under it at all, in either store. ⚠️ `blobs/` and `spaces/_engine/` in R2, and `users:secrets` in KV, are deliberately NOT under it — the first two are shared by decision, the third is account-level.",
  },
  {
