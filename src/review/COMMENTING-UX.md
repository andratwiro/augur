# Commenting UX — Figma-style compose & thread (build plan)

Target spec for upgrading the review overlay (`comments.js`) to match Figma's
commenting interaction model, captured from the reference board
(*CitizenLab Work — Sonder*). This is a **build plan**, not a description of
...
- On submit: persist via `mutate({op:"add", …})` (comments.js:522), collapse the
  compose, and render the saved pin (numbered) + open thread, matching Figma which
  drops you into the open thread after first send.
...
- Build: add the toolbar row + send-arrow button; wire ⏎/Shift+⏎ key handling on
  ...
  move out of the inline form — Figma identifies the author by avatar, not a text
  field. Capture the name **once** (first-run prompt or a small identity chip), then
...
- Picking an emoji inserts it at the caret and (Figma behaviour) keeps the picker
...
**Header icon mapping (our semantics — not Figma's):**
...
- **⋯ = Delete.** It is *not* an overflow menu — clicking it deletes the thread
  (keep the existing confirm, comments.js:568–571). Style it as Figma's three-dot
...
- Reply: collapsed grey **"Reply"** pill with inner send arrow (disabled until text);
  ...
  sends, Shift+⏎ newline).
- **Relative time:** Figma shows "Just now" / "1 minute ago". Replace the absolute
...
2. **Theme — DECIDED: white only.** Remove the `prefers-color-scheme: dark` block
   (comments.js:282–292). Go full-Figma white for all compose/thread chrome.
