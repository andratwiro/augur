 ...
 * human name so agents and humans share a vocabulary ("the onboarding tile"), not pixel pointing.
 ...
 *                    rotate pads) · rotation (node.rot, a CSS transform on the host)
 ...
 *   interaction      pointer (pan/move/resize/rotate/marquee/pinch, deferred double-tap) ·
 ...
 *
 * Deep docs + the load-bearing gotchas: CANVAS.md at the engine root.
 */
(function () {
  "use strict";
  ...
  var IMG_MAX_DIM = 1400, IMG_QUALITY = 0.55; // aggressive on purpose: size over quality
  ...
  // own-property lookups only: `stamp` is an untrusted string off the wire, so a bare
  // STAMP_ART[key] would happily hand back Object.prototype's members
  function stampArt(key) { return Object.prototype.hasOwnProperty.call(STAMP_ART, key) ? STAMP_ART[key] : null; }
  function stampHtml(key) { var a = stampArt(key); return a && a.indexOf("<svg") === 0 ? a : null; }
  // the emoji a legacy stamp painted with — either a known art string or the char the old
  // boards stored directly (clipped to a couple of glyphs, never arbitrary text)
  function stampChar(key) { var a = stampArt(key); return a || (typeof key === "string" ? key.slice(0, 4) : ""); }
  // The FACE stamp: the same die-cut circle every other sticker wears, filled with a person's
  // avatar. Drawn in the 40×40 sticker viewBox so it scales with the node like the rest.
  // ⚠️ src is interpolated into an innerHTML string and the node did NOT necessarily come from
  // this client (boards round-trip through KV and the room socket) — it goes through clipPath
  // at every call site, same rule as renderDraw's colors. No src ⇒ initials on the chip color.
  var avSeq = 0;
  function initialsOf(name) {
    var parts = String(name || "").trim().split(/\s+/).filter(Boolean);
    // first + last initial for a full name, ONE letter for a single word — same shape the
    // presence chips wear, so a face-less stamp still reads as that person
    var s = parts.length > 1 ? parts[0].charAt(0) + parts[parts.length - 1].charAt(0) : (parts[0] || "").charAt(0);
    return s.toUpperCase().replace(/[^A-Z0-9]/g, "") || "?";
  }
  function avatarSvg(src, name, color) {
    var ring = '<circle cx="20" cy="20" r="16.6" fill="none" stroke="#ffffff" stroke-width="3.6"/>';
    if (src) {
      var cid = "gvcav" + (++avSeq);
  ...
  // Does this avatar path actually resolve? One probe per distinct src, however many stamps
  // wear that face; callers get the answer immediately once it's known (see renderStamp).
  var FACE_OK = Object.create(null);
  // true / false once known, null while unknown. Callers that render BEFORE they are in the
  // DOM (every renderNode caller — the host is appended after the render returns) must paint
  // a known-good face immediately rather than wait for a callback: faceProbe answers
  // synchronously off this cache, and a synchronous callback has no attached host to write to.
  function faceCached(src) { var st = FACE_OK[src]; return st === true || st === false ? st : null; }
  function faceProbe(src, cb) {
    var st = FACE_OK[src];
    if (st === true || st === false) return cb(st);
    if (st) return st.push(cb);
    var waiting = FACE_OK[src] = [cb];
    var img = new Image();
    img.onload = function () { FACE_OK[src] = true; waiting.forEach(function (f) { f(true); }); };
    img.onerror = function () { FACE_OK[src] = false; waiting.forEach(function (f) { f(false); }); };
    img.src = src;
  }
  ...
  // data-gv-chrome: these are the board's INSTRUMENTS, not its content. The review
  // overlay leaves them clickable while you place comments (see comments.js) — you still
  // need to zoom, switch tool and leave comment mode with the mouse.
  var ui = el("div", { id: "gvc-ui", "data-gv-chrome": "" });
  ...
  function removeNode(id) {
    ...
    selected = selected.filter(function (s) { return s !== id; }); refreshSelBar();
    scheduleSave();
  }
  ...
  function selNodes() {
    var out = [];
    selected.forEach(function (id) { var n = nodeById(id); if (n) out.push(n); });
    return out;
  }
  // The type a MULTI-selection can be styled as one — null when it's a mixed bag. A control
  // that would mean different things to different nodes has no business on the bar.
  function uniformType(nodes) {
    if (!nodes.length) return null;
    for (var i = 1; i < nodes.length; i++) if (nodes[i].type !== nodes[0].type) return null;
    return nodes[0].type;
  }
  function refreshSelBar() { showSelBar(selNodes()); }
  function setSelection(ids) {
    ...
    if (selected.length === 1) decorate(selected[0]); // resize handles are a single-node affair
    // EVERY node type gets the bar — the ones with no styling controls (image, table, stamp,
    // arrow) still need somewhere to hang "copy link to this node" — and so does a MULTI
    // selection, which styles the lot at once (see showSelBar). The one exception is mid-
    // marquee: the hit list changes on every pointermove, and a bar rebuilt (and re-centred)
    // that often is a strobe light over the very nodes you're sweeping. It lands on pointerup.
    if (!selected.length || (drag && drag.mode === "marquee")) hideSelBar();
    else refreshSelBar();
  }
    ...
    if (!node.locked) {
      ...
      if (ROTATABLE[node.type]) {
        ["nw", "ne", "sw", "se"].forEach(function (dir) {
          var rt = el("div", { class: "gvc-rot " + dir, title: "Drag to rotate (Shift = 15°)" });
          rt.addEventListener("pointerdown", function (e) { startRotate(e, node); });
          rt.addEventListener("dblclick", function (e) { e.stopPropagation(); setRot(node, 0); scheduleSave(); });
          host.appendChild(rt); decorEls.push(rt);
        });
      }
  ...
  function scaleDecor() {
    ...
    decorEls.forEach(function (e) {
      // a rotate pad hangs outside its corner; its transform-origin is that corner (CSS),
      // so the same counter-scale keeps it a constant screen size AND still hugging it
      if (e.classList.contains("gvc-rot")) { e.style.transform = "scale(" + inv + ")"; return; }
  ...
  // ---- rotation (node.rot, degrees, about the box centre) -------------------
  // A rotated node is JUST a CSS transform on the host — every child (sticky text, an
  // image, a live tile's iframe) comes along, and hit-testing follows for free because the
  // browser transforms pointer coordinates too. The angle rides a custom property rather
  // than an inline transform so the pop animation can compose with it (see gvc-pop), and
  // the `rot` class is only added when there IS an angle: an unconditional transform would
  // make a containing block + stacking context out of every node on the board.
  var ROTATABLE = { sticky: 1, text: 1, image: 1, tile: 1, draw: 1, shape: 1, table: 1, stamp: 1 };
  function rotOf(node) { var r = +(node && node.rot) || 0; return r > -0.05 && r < 0.05 ? 0 : r; }
  function applyRot(host, node) {
    if (!host) return;
    var r = rotOf(node);
    if (r) { host.style.setProperty("--gvc-rot", r + "deg"); host.classList.add("rot"); }
    else { host.style.removeProperty("--gvc-rot"); host.classList.remove("rot"); }
  }
  ...
  // The editable currently being typed in, whichever node it belongs to.
  function liveEditable() { return world.querySelector('.gvc-txt[contenteditable="true"]'); }
  // LEAVE edit mode, deliberately. Blur alone used to be the only way out, and it was reachable
  // only by clicking some OTHER element — Escape left the node editing, so it stayed in a mode
  // with no visible difference and no way to move it (a press on the text belongs to the caret,
  // and the rest of the node bailed out of the drag path).
  // ⚠️ The SELECTION has to go with it. enterEdit selects the whole text, and a selection that
  // outlives edit mode turns the next press on those glyphs into a native drag of the text —
  // Chromium lifts a ghost image of it and the node itself never moves.
  function exitEdit() {
    var t = liveEditable(); if (!t) return false;
    var sel = getSelection();
    if (sel && sel.rangeCount && t.contains(sel.anchorNode)) sel.removeAllRanges();
    t.blur(); // the blur handler commits the text, renames, autofits and clears .editing
    return true;
  }
  ...
  function toggleFormat(node, btn, cmd, prop, targets) {
    var t = editingTxt(node);
    if (t) { execRich(cmd); commitRich(node, t); autoFit(node, true); scheduleSave(); return; }
    // One value for the whole selection, read off `node` — never a per-node flip, which would
    // leave half of what you selected bold and the other half not.
    var want = !node[prop];
    (targets || [node]).forEach(function (n) { n[prop] = want; applyNodeStyle(n); });
  ...
  function editableText(node, host, cls) {
    ...
    txt.addEventListener("keydown", function (e) {
      e.stopPropagation();
      // Escape LEAVES the text. It has to be handled here: this listener swallows every keydown
      // so canvas shortcuts don't fire mid-typing, which also swallowed the Escape that was
      // supposed to end the edit — so the only way out was clicking some other element, and a
      // node nobody knew was still editing could not be moved or nudged. The node stays
      // selected (a second Escape, now reaching the canvas, deselects it).
      if (e.key === "Escape") {
        e.preventDefault();
        var id = node.id;
        exitEdit();
        if (nodeById(id)) select(id); // an emptied text node deletes itself on the way out
        return;
      }
  ...
  // ~~strike~~ convert the moment you close the delimiter, and "->" becomes "→". The typed
  // markers are eaten.
  var MD_INLINE = [
    { re: /(^|[\s(])\*\*([^*\n]+)\*\*$/, cmd: "bold", mark: 2 },
    { re: /(^|[\s(])__([^_\n]+)__$/, cmd: "bold", mark: 2 },
  ...
  function autoFormat(node, txt, ev) {
    ...
    // 2. arrow — "->" (or "-->") becomes "→" as the ">" lands. Boards are full of typed
    // arrows; the glyph is what was meant, and it survives the copy out of the canvas.
    var arrow = typed === ">" && /(-{1,2})>$/.exec(at.nodeValue.slice(0, off));
    if (arrow) {
      var eatArrow = document.createRange();
      eatArrow.setStart(at, off - arrow[0].length); eatArrow.setEnd(at, off);
      s.removeAllRanges(); s.addRange(eatArrow);
      execRich("insertText", "→");
  ...
  function enterEdit(id) {
    ...
    // ⚠️ preventScroll on EVERY focus() inside the world. The camera is a transform on
    // #gvc-world; the browser knows nothing about it and answers "reveal the focused element"
    // the only way it can — by scrolling an ancestor. #gvc-root is `overflow: clip` now so
    // there is nothing to scroll, and this says the same thing at the call site: editing a
    // node never moves the viewport (if the node needs framing, that's flyTo's job).
    txt.contentEditable = "true"; txt.focus({ preventScroll: true });
  ...
  function wireRename(nm, node) {
    ...
    nm.addEventListener("pointerdown", function (e) {
      ...
      e.stopPropagation();
      var now = Date.now();
      if (now - tap < 350) { tap = 0; nm.contentEditable = "true"; nm.focus({ preventScroll: true }); if (document.execCommand) document.execCommand("selectAll", false, null); }
  ...
  function mountTile(node) {
    ...
    frame.addEventListener("load", function () { themeFrame(frame); mpFrameLoad(node, frame); applyView(node, frame); });
  ...
  function exitInteract() {
    ...
    if (isSelected(id)) refreshSelBar();
  }
  ...
  function setDevice(node, d) {
    ...
    scaleTileChrome(node, host);
    if (isSelected(node.id)) refreshSelBar();
    positionSelBar();
    scheduleSave();
  }
  ...
  // ---- framed view (node.viewAt) -------------------------------------------
  // A tile remembers WHERE IN THE PAGE it was left, the same way node.liveUrl remembers
  // which page: scroll a prototype while driving it and that view becomes the tile's, for
  // the room and for every later mount (reload, late joiner, LRU remount) — until the next
  // person scrolls it. Only a DRIVER writes it (interactId), so a prototype scrolling itself
  // on load can't claim the view. Shape mirrors the demo-sync scroll event so one capture
  // serves both: {u: page the view was taken on, sel: "@win"|css, top, left}. The url stamp
  // is what keeps a stale view off a page it wasn't taken on.
  function viewSig(v) { return v ? (v.u || "") + "|" + (v.sel || "") + "|" + (v.top || 0) + "|" + (v.left || 0) : ""; }
  function rememberView(node, ev, win, driving) {
    if (!driving) return; // only the person driving reframes the tile (the caller reads that
                          // off the scroll EVENT — by now Stop may already have been pressed)
    var va = { u: win.location.pathname + win.location.search + win.location.hash, sel: ev.sel, top: ev.top || 0, left: ev.left || 0 };
    if (viewSig(node.viewAt) === viewSig(va)) return;
    node.viewAt = va;
    histIgnore(node, "viewAt"); // a scroll is not an undo step (undo re-renders → reloads the frame)
    scheduleSave(); // the diff tick versions + broadcasts it like any other node change
  }
  // Restore on mount. Retried on a short ramp because the page settles late (images, fonts,
  // an inner scroller that doesn't exist yet at load) — but never against the person driving,
  // and never after the stored view has moved on.
  function applyView(node, frame, delay) {
    var va = node.viewAt; if (!va) return;
    var win, doc;
    try { win = frame.contentWindow; doc = win.document; if (!doc) return; } catch (e) { return; }
    if (va.u && va.u !== win.location.pathname + win.location.search + win.location.hash) return; // view belongs to another page
    var ramp = [0, 120, 350, 800], i = delay == null ? 0 : delay;
    (function step() {
      if (i >= ramp.length) return;
      var at = ramp[i++];
      setTimeout(function () {
        if (!frame.isConnected || node.viewAt !== va) return;   // gone, or someone reframed it
        if (at && interactId === node.id) return;               // you're driving — don't yank the page
        try {
          win.__mpQuiet = Date.now() + 200; // a restore is not a scroll of ours to mirror
          if (va.sel === "@win") win.scrollTo(va.left || 0, va.top || 0);
          else { var se = va.sel && doc.querySelector(va.sel); if (se) { se.scrollTop = va.top || 0; se.scrollLeft = va.left || 0; } }
        } catch (e) {}
        step();
      }, at);
    })();
  ...
  function editCell(node, cellEl) {
    select(node.id);
    cellEl.contentEditable = "true"; cellEl.focus({ preventScroll: true });
  ...
  function renderStamp(node) {
    ...
    // a face stamp carries the FACE OF WHOEVER STAMPED IT (src/name/color on the node) —
    // never the viewer's own, or every board would show you your own head everywhere
    if (node.stamp === "avatar") {
      // Initials FIRST, the photo swapped in only once it has really loaded. A face stamp
      // stores the avatar path as it was the day someone pressed it, and that path is only
      // valid against the identity the instance serves now — a seed photo dropped from the
      // roster (or any change to one) 404s it on a board that may be months old. An <image>
      // that 404s leaves the die-cut ring with a HOLE in it; initials leave a person.
      // ⚠️ A face already KNOWN to load is painted in the first innerHTML, never through the
      // callback. renderStamp runs before its caller attaches the host (renderNode appends
      // what the render returns), and faceProbe answers a cached src SYNCHRONOUSLY — so the
      // `isConnected` guard below, correct for a real async load, silently threw away the
      // photo for every face stamp after the first: the initials stuck for good, and your own
      // head turned into your initials the moment you pressed a second sticker.
      var face = clipPath(node.src), fname = node.name, fcolor = clipColor(node.color);
      var known = face ? faceCached(face) : false;
      host.innerHTML = avatarSvg(known === true ? face : "", fname, fcolor);
      if (face && known === null) faceProbe(face, function (ok) { if (ok && host.isConnected) host.innerHTML = avatarSvg(face, fname, fcolor); });
    } else {
      var art = stampHtml(node.stamp);
      if (art) host.innerHTML = art;
      else {
        host.textContent = stampChar(node.stamp) || "👍";
  ...
  // stamps land with a slight tilt, like a sticker pressed on by hand — the whole point of
  // the sticker set is that it doesn't look laid out. Never exactly 0 (that reads as aligned).
  function stampTilt() { var t = (Math.random() * 9 + 3) * (Math.random() < 0.5 ? -1 : 1); return Math.round(t * 10) / 10; }
  ...
  root.addEventListener("pointerdown", function (e) {
    ...
    // A press on an EDITING node: inside its text the browser owns the gesture (caret, and
    // dragging selects) — the editable's own pointerdown handler has already stopped that from
    // reaching here. Anywhere else on the node means "I'm done typing, now move me": leave edit
    // and fall through to the normal drag. It used to bail out instead, so an editing node was
    // simply immovable — and nothing on screen said why.
    if (nodeHost && nodeHost.classList.contains("editing")) {
      if (!exitEdit()) return;
    }
    if (pan) {
      mpUnfollow();
  ...
  function nodeRect(n) {
    ...
    if (!w || !h) return null;
    // a rotated node's rect is the AXIS-ALIGNED box it actually occupies — that's what
    // snapping, section membership and "fly to this node" all mean by a node's rect
    var r = rotOf(n);
    if (r) {
      var c = Math.abs(Math.cos(r * Math.PI / 180)), s = Math.abs(Math.sin(r * Math.PI / 180));
      var bw = w * c + h * s, bh = w * s + h * c;
      return { x: n.x + (w - bw) / 2, y: n.y + (h - bh) / 2, w: bw, h: bh };
    }
  ...
  root.addEventListener("pointermove", function (e) {
    ...
    if (drag.mode === "pan") { board.view.x = drag.ox + dx; board.view.y = drag.oy + dy; applyTransform(); }
      ...
      // a rotated node resizes along its OWN axes: rotate the pointer delta into node space
      // first (identity when rot is 0, so an upright node is untouched by this)
      var rr = rotOf(n2) * Math.PI / 180, rc = Math.cos(rr), rs = Math.sin(rr);
      var ldx = rr ? dx * rc + dy * rs : dx, ldy = rr ? -dx * rs + dy * rc : dy;
      var nw2 = doW ? Math.max(MIN_NODE, drag.ow + (west ? -ldx : ldx) / sc) : drag.ow;
      var nh2 = doH ? Math.max(MIN_NODE, drag.oh + (north ? -ldy : ldy) / sc) : drag.oh;
      ...
      } else if (rr) { clearGuides(); /* the guides are axis-aligned; a tilted box has no edge to latch */ }
      ...
      // Keep the side/corner OPPOSITE the one you grabbed pinned in world space. CSS rotates
      // about the centre, so that anchor has to be expressed through the centre: with the
      // anchor's local offset a (± half-size, signs fixed by the grabbed side), the new centre
      // is c0 + R·(a_before − a_after). At rot 0 this reduces to the old
      // "x = ox + (ow − w)" / "y = oy + (oh − h)", so upright resizing is unchanged.
      var aSgnX = west ? 1 : (east ? -1 : 0);            // grabbed west ⇒ the EAST side is the anchor
      var aSgnY = north ? 1 : ((d === "s" || d === "sw" || d === "se") ? -1 : 0);
      var hNew = n2.type === "text" ? drag.oh : nh2;     // a text box owns its height — never resized here
      var adx = aSgnX * (drag.ow - nw2) / 2, ady = aSgnY * (drag.oh - hNew) / 2;
      n2.x = drag.ox + drag.ow / 2 + (rr ? adx * rc - ady * rs : adx) - nw2 / 2;
      n2.y = drag.oy + drag.oh / 2 + (rr ? adx * rs + ady * rc : ady) - hNew / 2;
      re.style.left = n2.x + "px"; re.style.top = n2.y + "px";
      ...
      // the angle is the pointer's bearing from the node's CENTRE, offset by the bearing it
      // had when the drag started — so the node never jumps to meet the cursor
      var ra = Math.atan2(e.clientY - drag.cy, e.clientX - drag.cx) * 180 / Math.PI;
      var deg = drag.rot0 + (ra - drag.ang0);
      deg = ((deg + 180) % 360 + 360) % 360 - 180;
      if (e.shiftKey) deg = Math.round(deg / 15) * 15;                       // Shift: 15° detents
      else { var near = Math.round(deg / 90) * 90; if (Math.abs(deg - near) < 2) deg = near; } // light stick to the square angles
      setRot(drag.node, deg);
      showRotBadge(e.clientX, e.clientY, rotOf(drag.node));
      positionSelBar();
    }
  ...
  function onPointerEnd(e) {
    ...
    if (drag.mode === "rotate") { hideRotBadge(); positionSelBar(); }
    ...
    var wasPan = drag.mode === "pan", wasMarquee = drag.mode === "marquee";
    drag = null;
    if (wasMarquee) refreshSelBar(); // the bar setSelection held back mid-sweep — `drag` had to clear first
  ...
  // ---- rotate drag ---------------------------------------------------------
  // rot is stored in DEGREES and normalised to (−180, 180]. It is written as 0 rather than
  // deleted when a node is straightened: `rot` is a GEO_KEY, and the geometry fast-path
  // copies keys that are present — a deleted one would never reach a peer's copy.
  function setRot(node, deg) {
    node.rot = Math.round(deg * 10) / 10;
    applyRot(nodeEls[node.id], node);
  }
  function startRotate(e, node) {
    e.stopPropagation();
    var h = nodeEls[node.id];
    var w = node.w != null ? node.w : (h ? h.offsetWidth : 0), hh = node.h != null ? node.h : (h ? h.offsetHeight : 0);
    var c = worldToScreen(node.x + w / 2, node.y + hh / 2);
    drag = { mode: "rotate", node: node, cx: c.x, cy: c.y, rot0: rotOf(node),
      ang0: Math.atan2(e.clientY - c.y, e.clientX - c.x) * 180 / Math.PI, sx: e.clientX, sy: e.clientY };
    root.setPointerCapture(e.pointerId);
  }
  var rotBadge = null;
  function showRotBadge(x, y, deg) {
    if (!rotBadge) { rotBadge = el("div", { id: "gvc-rotbadge" }); root.appendChild(rotBadge); }
    rotBadge.textContent = (deg > 0 ? "+" : "") + (Math.round(deg * 10) / 10) + "°";
    rotBadge.style.left = (x + 18) + "px"; rotBadge.style.top = (y + 18) + "px";
    rotBadge.classList.remove("hidden");
  }
  function hideRotBadge() { if (rotBadge) rotBadge.classList.add("hidden"); }
  ...
  var NUDGE = 1, NUDGE_BIG = 10;
  var NUDGE_KEYS = { ArrowLeft: [-1, 0], ArrowRight: [1, 0], ArrowUp: [0, -1], ArrowDown: [0, 1] };
  function nudge(dx, dy) {
    var moving = withSectionChildren(selected); // a section carries its contents, same as a drag
    var moved = false;
    moving.forEach(function (id) {
      var n = nodeById(id); if (!n) return;
      moved = true;
      if (n.type === "arrow") { n.x1 += dx; n.y1 += dy; n.x2 += dx; n.y2 += dy; renderNode(n); }
      else { n.x += dx; n.y += dy; var h = nodeEls[id]; if (h) { h.style.left = n.x + "px"; h.style.top = n.y + "px"; } }
    });
    if (!moved) return false;
    positionSelBar();
    // Same debounced save + history tick a drag ends on, so a burst of key-repeat lands as ONE
    // undo step and one write rather than sixty.
    scheduleSave();
    return true;
  }
  document.addEventListener("keydown", function (e) {
    ...
    // Nudge sits ABOVE the tool shortcuts: with Shift held those read the key as a letter
    // chord and return, which would swallow ⇧← as surely as if it were unhandled.
    var nk = NUDGE_KEYS[e.key];
    if (nk && !e.metaKey && !e.ctrlKey && !e.altKey) {
      if (nudge(nk[0] * (e.shiftKey ? NUDGE_BIG : NUDGE), nk[1] * (e.shiftKey ? NUDGE_BIG : NUDGE))) e.preventDefault();
      return;
    }
    if (e.metaKey || e.ctrlKey || e.altKey) return;
    // tool shortcuts
  ...
  // Nodes are NEVER a native HTML5 drag source — NOT EVEN WHILE BEING EDITED. The browser will
  // happily start one of its own from a node's text run or an <img>, which paints a ghost copy
  // under the cursor and fires dragover, lighting up the "drop image" overlay in the middle of
  // an ordinary node drag. The canvas moves nodes with its own pointer handlers, so kill it at
  // the source.
  //
  // ⚠️ An earlier version let an EDITING node through, on the theory that dragging a text
  // selection around inside the box is a real affordance. It isn't, and it costs a lot: a text
  // node in edit mode has all its text selected, so pressing inside it and dragging — exactly
  // what you do when you mean to MOVE the box you just typed into — hands the browser a
  // selection drag instead. Chromium paints that drag image from the DOM WITHOUT the world's
  // zoom transform, so an 80px "Huge" text node smears a translucent, page-sized copy of itself
  // across the board while the node itself doesn't budge. Dragging inside an editing box now
  // does what it does everywhere else: it changes the text selection.
  root.addEventListener("dragstart", function (e) {
    if (e.target && e.target.closest && e.target.closest(".gvc-node")) e.preventDefault();
  });
  ...
  function clipSanitize(raw) {
    ...
    var rt = clipNum(raw.rot, 0); if (rt) n.rot = Math.max(-180, Math.min(180, rt));
    ...
    if (n.type === "image") {
      ...
      // a duplicated tile keeps its framed view, the same way it keeps liveUrl
      if (raw.viewAt && typeof raw.viewAt === "object") {
        var vsel = clipStr(raw.viewAt.sel, 400);
        if (vsel) {
          n.viewAt = { sel: vsel, top: clipNum(raw.viewAt.top, 0), left: clipNum(raw.viewAt.left, 0) };
          var vu = clipPath(raw.viewAt.u); if (vu) n.viewAt.u = vu;
        }
      }
      ...
      // hasOwnProperty, not a bare lookup: `stamp` is an untrusted string and "constructor"
      // would otherwise pass as a legacy key
      n.stamp = (STAMPS.indexOf(raw.stamp) >= 0 || Object.prototype.hasOwnProperty.call(LEGACY_STAMPS, raw.stamp)) ? raw.stamp : STAMPS[0];
      if (n.stamp === "avatar") { var av = clipPath(raw.src); if (av) n.src = av; } // the face it was stamped with travels with it
  ...
  // The bar styles the WHOLE selection, not just one node: ten stickies go blue in one click,
  // a run of text nodes takes one size. Two rules keep that honest — a control only appears
  // when every selected node is the same TYPE (a colour means something different to a sticky
  // and to a drawing), and every control READS its current state off the first selected node
  // and WRITES the same value to all of them. So a click is never a per-node toggle that
  // leaves the selection half-on, half-off. Rich-text controls (the two list buttons) stay
  // single-only: they need a live editable, and entering an edit collapses the selection.
  function showSelBar(nodes) {
    if (!nodes || !nodes.length) { hideSelBar(); return; }
    selBar.innerHTML = "";
    var node = nodes[0], many = nodes.length > 1;
    var type = many ? uniformType(nodes) : node.type; // null = mixed bag: universal actions only
    var each = function (fn) { nodes.forEach(fn); };
    ...
    if (type === "tile") {
      ["desktop", "tablet", "phone"].forEach(function (d) {
        ...
        guard(b); b.addEventListener("click", function (e) { e.stopPropagation(); each(function (n) { setDevice(n, d); }); });
        selBar.appendChild(b);
      });
      // driving a prototype and opening it are about ONE frame — never fanned out
      if (!many) {
        ...
        guard(ib); ib.addEventListener("click", function (e) { e.stopPropagation(); if (interactId === node.id) exitInteract(); else enterInteract(node); refreshSelBar(); });
        selBar.appendChild(ib);
        // Open the prototype itself, outside the frame. Also the diagnosis button: an embed
        // that renders the login page (any URL the gate doesn't know is public answers that
        // way, 404s included) looks identical to a broken one, and this says which — the
        // page opening fine in a tab means the frame is stale, not the prototype gone.
        var ob = el("button", { type: "button", class: "btn", title: "Open the prototype in a new tab", html: lucideIcon(I_EXTERNAL) });
        guard(ob); ob.addEventListener("click", function (e) { e.stopPropagation(); window.open(node.url, "_blank", "noopener"); });
        selBar.appendChild(ob);
      }
      addLinkBtn(node, many);
    ...
    if (PALETTED[type]) {
      var dot = el("div", { class: "dot" }); dot.style.background = node.color || (type === "draw" || type === "text" ? "#1e1e1e" : type === "shape" ? "#ffffff" : type === "section" ? "#c4c9d4" : DEFAULT_STICKY);
      var sw = el("div", { class: "sw" }, [dot, el("div", { class: "chev", text: "▾" })]);
      guard(sw); sw.addEventListener("click", function (e) { e.stopPropagation(); togglePalette(node, dot, nodes); });
    ...
    if (type === "section") {
      ...
      guard(lb); lb.addEventListener("click", function (e) { e.stopPropagation(); toggleLockMenu(node, lb, nodes); });
    ...
    if (type === "sticky" || type === "shape" || type === "text") {
      ...
      if (type === "sticky" || type === "text") {
        ...
        guard(fb); fb.addEventListener("click", function (e) { e.stopPropagation(); toggleFontMenu(node, fb, nodes); });
      ...
      guard(bb); keepFocus(bb); bb.addEventListener("click", function (e) { e.stopPropagation(); toggleFormat(node, bb, "bold", "bold", nodes); });
      ...
      guard(itb); keepFocus(itb); itb.addEventListener("click", function (e) { e.stopPropagation(); toggleFormat(node, itb, "italic", "italic", nodes); });
      ...
      guard(stb); keepFocus(stb); stb.addEventListener("click", function (e) { e.stopPropagation(); toggleFormat(node, stb, "strikeThrough", "strike", nodes); });
      selBar.appendChild(stb);
      // lists: the selected lines (or the whole box when the node isn't being edited). Single
      // node only — toggleList works through the live editable, and entering an edit selects
      // that one node, which would silently drop the rest of a multi-selection.
      if (!many) {
      ...
      if (type === "text") {
        ...
        guard(alb); alb.addEventListener("click", function (e) {
          e.stopPropagation();
          var next = ALIGN[(ALIGN.indexOf(node.align || "left") + 1) % 3]; // one value, off the first node, for all of them
          each(function (n) { n.align = next; applyNodeStyle(n); });
          alb.innerHTML = lucideIcon(AL_ICON[next]); scheduleSave();
        });
  ...
  // to it. The picture already understood multi-selections; a link never can — it addresses
  // ONE node id — so it's dropped rather than made to pick a favourite.
  function addLinkBtn(node, many) {
  ...
  function togglePalette(node, dot, targets) {
    ...
    colors.forEach(function (c) {
      ...
      guard(pc); pc.addEventListener("click", function (e) {
        e.stopPropagation();
        (targets || [node]).forEach(function (n) { n.color = c; applyNodeStyle(n); });
        dot.style.background = c; scheduleSave(); palette.classList.add("hidden");
      });
  ...
  function toggleLockMenu(node, btn, targets) {
    ...
    items.forEach(function (it) {
      ...
      guard(row); row.addEventListener("click", function (e) {
        e.stopPropagation();
        (targets || [node]).forEach(function (n) {
          if (it[0]) n.locked = it[0]; else delete n.locked;
          renderNode(n);
        });
        setSelection(selected.slice()); scheduleSave(); // re-render dropped the "sel" class
  ...
  function setFontSize(node, px, btn, targets) {
    var v = Math.max(6, Math.min(400, px));
    (targets || [node]).forEach(function (n) { n.fontSize = v; delete n.fontScale; applyNodeStyle(n); });
    scheduleSave();
    if (btn) { var lbl = btn.querySelector(".fslabel"); if (lbl) lbl.textContent = fontLabel(node); }
    positionSelBar();
  }
  function toggleFontMenu(node, btn, targets) {
    ...
    FONT_PRESETS.forEach(function (p) {
      ...
      row.appendChild(lb);
      guard(row); row.addEventListener("click", function (e) { e.stopPropagation(); setFontSize(node, p.px, btn, targets); fontMenu.classList.add("hidden"); });
      fontMenu.appendChild(row);
    });
    var inp = el("input", { class: "num", type: "number", min: "6", max: "400", value: String(cur) });
    guard(inp);
    function applyNum() { setFontSize(node, parseInt(inp.value, 10) || cur, btn, targets); }
  ...
  // The union box of the selection — what a multi-selection's bar centres on. anyRect, not
  // node.x/node.w: arrows carry endpoints instead, and they get a bar too.
  function selBarRect() {
    var x1 = Infinity, y1 = Infinity, x2 = -Infinity, y2 = -Infinity, seen = false;
    selected.forEach(function (id) {
      var n = nodeById(id); if (!n) return;
      var r = anyRect(n) || { x: n.x || 0, y: n.y || 0, w: n.w || 150, h: n.h || 0 };
      seen = true;
      x1 = Math.min(x1, r.x); y1 = Math.min(y1, r.y);
      x2 = Math.max(x2, r.x + (r.w || 0)); y2 = Math.max(y2, r.y + (r.h || 0));
    });
    return seen ? { x: x1, y: y1, w: x2 - x1, h: y2 - y1 } : null;
  }
  function positionSelBar() {
    if (!selBar || selBar.classList.contains("hidden")) return;
    var r = selBarRect(); if (!r) { hideSelBar(); return; }
  ...
  var I_EXTERNAL = '<path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/>'; // external-link (Lucide)
  ...
  // per the zoomed reference asset: the FRONT sheet is upright with the
  ...
  function setStampFace(elm, key) {
    if (key === "avatar") { var f = myFace(); elm.innerHTML = avatarSvg(f.src, f.name, f.color); return; }
    var art = stampHtml(key);
    if (art) elm.innerHTML = art;
    else elm.textContent = stampChar(key);
  }
  // identity arrives asynchronously (/__me, or the room handing out your colour), so the face
  // slot is repainted when it lands — otherwise the wheel keeps the initials it booted with
  function refreshFaceStamp() {
    stampEls.forEach(function (b) { if (b.dataset.s === "avatar") setStampFace(b, "avatar"); });
    if (toolGhost && armedStamp === "avatar" && toolGhost.classList.contains("gvc-ghost-stamp")) setStampFace(toolGhost, "avatar");
  }
  ...
  function buildUI() {
    ...
    nm.addEventListener("click", function () { if (nm.contentEditable !== "true") { nm.contentEditable = "true"; nm.focus({ preventScroll: true }); document.execCommand("selectAll", false, null); } });
    ...
    syncBars();
    // phone: you're navigating, not editing — pan by default,
  ...
  function openPicker() {
    ...
    var inp = picker.querySelector("input"); inp.value = ""; pickQuery = ""; inp.focus({ preventScroll: true });
  ...
  var sessPill, sessPillTime, sessPillIcon, sessPillSpk, sessPanel, sessDigits, sessIdle, sessRun, sessPauseBtn, sessMusicBody, sessDeck, sessTrackBtn, sessTrackMenu, sessTrackChoice, sessPlayBtn, sessVolIn, sessVolIcon;
  var I_CHEV = '<path d="m6 9 6 6 6-6"/>'; // chevron-down (Lucide)
  var I_TIMER = '<line x1="10" x2="14" y1="2" y2="2"/><line x1="12" x2="15" y1="14" y2="11"/><circle cx="12" cy="14" r="8"/>'; // timer (Lucide)
  ...
  var sessTickSec = null;    // last whole second we sounded, so the tick-tock fires once each
  ...
  function buildSession() {
    // pill: timer glyph + seven-segment time (or track name), plus a per-user quick-mute
    ...
    sessPillIcon = el("span", { class: "ico", html: lucideIcon(I_TIMER) });
  ...
  function sessTick() {
    ...
    // the last five seconds are audible: a whole tick-TOCK inside each one, so a room looking
    // anywhere but the pill still hears the end coming. Only on a RUNNING countdown — a paused
    // clock parked at 00:03 must not tick.
    var sec = Math.max(0, Math.ceil(r / 1000));
    if (sessRunning() && sec > 0 && sec <= 5 && sessTickSec != null && sec < sessTickSec) sessTickTock();
    sessTickSec = sec;
    if (r <= 0 && !sessRang) { sessRang = true; sessEnd(); }
  }
  ...
  // idle state and nobody has to broadcast at the moment it matters.
  function sessEnd() {
    var total = sess.timer && sess.timer.total;
    sessChime();
    if (total >= 1000) sessPending = Math.min(total, 99 * 60000 + 59000);
    sess = { timer: null, music: sess.music };
    sessTickSec = null;
    sessSyncTimer(true);
  }

  // The countdown's sounds are SYNTHESIZED, never assets — the engine ships no audio (the
  // music picker is a hook for a space's own tracks), and a tick that has to be downloaded
  // is a tick that arrives late. Both run through the same per-user volume/mute as the music.
  //
  // Only ever scheduled on a RUNNING AudioContext: a suspended one (this tab was never
  // interacted with, autoplay policy) would hold the notes and fire them stale on the next
  // click, so it's nudged awake for next time and this second stays silent.
  var sessAudioCtx = null;
  function sessCtx() {
    if (sessMuted || sessVol <= 0 || EMBEDDED) return null; // same reason as the music: a preview is silent
    try {
      if (!sessAudioCtx) sessAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      if (sessAudioCtx.state !== "running") { sessAudioCtx.resume().catch(function () {}); return null; }
      return sessAudioCtx;
    } catch (e) { return null; }
  }

  // One knock of the clock: a ~30ms burst of noise with a steep decay through a narrow
  // bandpass — an escapement, not a beep. tic and toc are the same body at two pitches.
  function sessKnock(ctx, hz, at, vol) {
    ...
    bp.type = "bandpass"; bp.frequency.value = hz; bp.Q.value = 5;
    var g = ctx.createGain(); g.gain.value = vol;
    src.connect(bp); bp.connect(g); g.connect(ctx.destination);
    src.start(at);
  }
  // Each of the last five seconds carries a WHOLE tick-tock, not one half of one:
  // "5 tick-tock · 4 tick-tock", the way a clock sounds in a second, rather than
  // a bare alternating knock that reads as a metronome. The tock lands 260ms behind its tick —
  // close enough to pair with it, far enough from the next second to leave the beat room.
  function sessTickTock() {
    var ctx = sessCtx();
    if (!ctx) return;
    try {
      var t0 = ctx.currentTime;
      sessKnock(ctx, 2400, t0, 0.9 * sessVol);
      sessKnock(ctx, 1600, t0 + 0.26, 0.75 * sessVol); // the tock sits a touch under the tick
    } catch (e) {}
  }

  // One struck bell: a sine fundamental plus a quiet INHARMONIC partial (2.76×, the first
  // mode of a real bar) and a whisper of noise for the mallet. That partial is the whole
  // difference between "glockenspiel" and "smoke alarm" — a pure sine reads as a beep.
  function sessBell(ctx, hz, at, vol) {
    [[hz, vol, 0.9], [hz * 2.76, vol * 0.12, 0.35]].forEach(function (p) {
      var o = ctx.createOscillator(), g = ctx.createGain();
      o.type = "sine"; o.frequency.value = p[0];
      g.gain.setValueAtTime(0.0001, at);
      g.gain.exponentialRampToValueAtTime(Math.max(0.0002, p[1]), at + 0.006); // struck, not swelled
      g.gain.exponentialRampToValueAtTime(0.0001, at + p[2]);
      o.connect(g); g.connect(ctx.destination);
      o.start(at); o.stop(at + p[2] + 0.05);
    });
  }
  // 00:00 itself: a bright three-note flourish up a major triad, mallet-struck and gone in
  // under a second. It has to close the round the ticks were counting toward without reading
  // as an ALARM — the point of the revert is that nothing is left demanding attention.
  function sessChime() {
    var ctx = sessCtx();
    if (!ctx) return;
    try {
      var t0 = ctx.currentTime, v = 0.34 * sessVol;
      [1174.7, 1480, 1760].forEach(function (hz, i) { // D6 · F#6 · A6
        sessBell(ctx, hz, t0 + i * 0.085, i === 2 ? v * 1.15 : v); // the last note lands hardest
      });
    } catch (e) {}
  }
  ...
  function sessRenderMusic() {
    ...
    // catalog): hide music entirely — panel row and title — rather than tease an inert
    // turntable that can't work there. It all returns when a catalog ships. (The pill is
    // the timer either way, so it says nothing about music.)
  ...
  function sessTrackMenuToggle(force) {
    ...
    if (open) {
      ...
      (sel || sessTrackMenu.firstChild).focus({ preventScroll: true });
    }
  }
  ...
  function sessApply(msg) {
    ...
    sessTickSec = null; // don't compare the new countdown's first second against the old one's
    sessSyncTimer(true);
  ...
  // hide ONE field's change from the diff (not the whole node — a pending local edit on the
  // same node must still reach the undo stack). Scrolling a tile is not an edit to undo:
  // an undo re-renders, and re-rendering a tile reloads its prototype.
  function histIgnore(node, k) { var sh = node && histShadow[node.id]; if (sh) sh[k] = node[k]; }
  ...
  function histApply(entry, dir) {
    ...
    if (selected.length) setSelection(selected.slice()); else hideSelBar();
    save();
    histBusy = false;
    return touched;
  }
  ...
  // tile signature ALSO ignoring live/liveUrl/viewAt — a change in only those is a live-state
  // transition handled in place (keep the iframe's DOM state), never a rebuild
  function mpTileSig(node) {
    var c = {}; for (var k in node) if (node.hasOwnProperty(k) && GEO_KEYS.indexOf(k) < 0 && k !== "live" && k !== "liveUrl" && k !== "viewAt") c[k] = node[k];
    return mpSig(c);
  }
  ...
  function mpRemoveLocal(id) {
    ...
    refreshSelBar(); // a peer deleting one of yours re-forms the bar around what's left
  }
    ...
    ops.forEach(function (op) {
      ...
      if (op.op === "upsert" && op.node && op.node.id) {
        ...
        if (local && host && mpGeoLessSig(r) === mpGeoLessSig(local) && local.type !== "arrow") {
          ...
          var reframed = viewSig(r.viewAt) !== viewSig(local.viewAt);
          local.viewAt = r.viewAt;
          place(host, local);
          ...
          if (fr) {
            ...
            // someone else reframed the tile — follow, unless you're the one driving it.
            // (While they're still scrolling you're already following via the proto relay;
            // this is what keeps the view once the relay goes quiet.)
            if (reframed && interactId !== local.id) applyView(local, fr);
          }
  ...
  function mpFrameLoad(node, frame) {
    ...
    var scT = null, scEl = null, scDrv = false;
    win.addEventListener("scroll", function (e) {
      ...
      scEl = e.target;
      // whether this was YOUR scroll is a fact about the event, not about 120ms later —
      // pressing Stop inside the throttle window must not lose the view you just set
      if (interactId === node.id) scDrv = true;
      if (scT) return;
      scT = setTimeout(function () {
        ...
        try {
          ...
          if (!scEl || scEl === doc || scEl === win || scEl === doc.documentElement || scEl === doc.body)
            ev = { k: "scroll", sel: "@win", top: win.scrollY, left: win.scrollX };
          else ev = { k: "scroll", sel: mpSelOf(scEl, doc), top: scEl.scrollTop, left: scEl.scrollLeft };
          mpSend({ t: "proto", id: node.id, ev: ev });
          rememberView(node, ev, win, scDrv); scDrv = false; // the live mirror is ephemeral; this is what the tile keeps
        } catch (err) {}
      }, 120);
    }, true);
  }
  ...
  // needs you (agents present the way PEOPLE do, one row for the
  ...
  function mpAgentPopOpen(sid, chip) {
    ...
    mpAgentPopSid = sid;
    mpAgentPopEl = el("div", { id: "gvc-agentpop", "data-gv-chrome": "" });
  ...
  function mpAgentPopSync() {
    ...
    var face = el("span", { class: "face", html: clawdChipSvg(p.color) });
    face.style.background = "#f6f0e4";
  ...
  function openCursorChat() {
    if (chatInput) { chatInput.querySelector("input").focus({ preventScroll: true }); return; }
  ...
  function mpDragGeo() {
    ...
    if (drag.mode === "move" && drag.items) {
      ...
      out.push(op);
    } else if (drag.mode === "rotate" && drag.node) {
      out.push({ id: drag.node.id, rot: rotOf(drag.node) });
  ...
  function mpRenderPresence() {
    ...
    // ONE row for the whole room: humans as initial chips, Clawds
    ...
    // your own chip shows even in an empty room — it's the proof the room
    // connected and that it's YOU in it; before, being alone hid it and you had to open a
    // second tab to see yourself
    mpPresence.classList.remove("hidden");
    chips.forEach(function (c) {
      var isAgent = c.kind === "agent";
      var chip = el("div", { class: "gvc-peerchip" + (c.me ? " me" : "") + (isAgent ? " agent" : "") });
      // every chip wears its owner's identity color; an agent chip flips it — the Clawd
      // itself is tinted the agent's color on a cream ground (the color IS the agent,
      // same as its cursor mascot) — humans wear the color as background, with their
      // photo when their account carries one, else their initial
      if (isAgent) { chip.style.background = "#f6f0e4"; chip.innerHTML = clawdChipSvg(c.color); }
      else if (c.avatar) chip.style.background = "url(\"" + c.avatar + "\") center/cover, " + c.color;
      else { chip.style.background = c.color; chip.textContent = mpInitials(c.name); }
  ...
  function mpOnMessage(ev) {
    ...
    if (m.t === "welcome") {
      ...
      if (m.color && m.color !== mpColor) { mpColor = m.color; mpApplyLocalCursor(); refreshFaceStamp(); } // your pointer (and your face stamp's fallback chip) take your room color
      mpPeers = {};
  ...
  // A FRAMED board is a picture of the room, not a seat in it: the index page's preview
  // tiles, a board embedded in another board. Joining puts a second copy of you in the
  // presence row (and a second cursor) for everyone in the real room — what you see when a
  // tab is parked on an opportunity screen whose preview happens to be a canvas. It still
  // renders and still reads the doc; it just doesn't announce itself.
  ...
  function mpBoot() {
    ...
    fetch("/__me", { headers: { Accept: "application/json" } })
      ...
      .then(function (d) {
        ...
        refreshFaceStamp(); // the wheel's face slot is YOU — repaint it now that we know who that is
